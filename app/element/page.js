...
export default function ElementPage() {
  ...
  const getCategoryColor = (category) => {
    switch (category) {
      case 'plan': return 'from-blue-400 to-blue-600';
      case 'do': return 'from-purple-400 to-purple-600';
      case 'check': return 'from-red-400 to-red-600';
  ...
  return (
    <div className="h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 lg:ml-0">
        ...
        <div className="bg-white shadow-sm border-b border-gray-200 p-4 lg:hidden">
          <button
          ...
          <div className="max-w-7xl mx-auto mb-8">
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Sistem komprehensif untuk memastikan keselamatan kerja melalui implementasi 
                12 elemen yang terintegrasi dalam siklus PDCA (Plan-Do-Check-Act)
              </p>
            </div>
              ...
              <div className="relative w-96 h-96">
                ...
                <div className="absolute inset-1/4 bg-white rounded-full shadow-lg border-4 border-gray-200 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-900 mb-2">SIKA</div>
                    <div className="text-sm text-gray-600">Sistem Izin Kerja Aman</div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {elements.map((element) => (
                ...
                >
                  <div className={`bg-gradient-to-r ${getCategoryColor(element.category)} p-4`}>
                  ...
                  <div className="p-6">
                    <div className="flex items-center mb-3">
                      <div className={`w-8 h-8 rounded-full ${element.color} text-white flex items-center justify-center font-bold text-sm mr-3`}>
                        {element.id}
                      </div>
                    ...
                    <div className="mt-4">
                      <div className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${element.color} text-white`}>
                        Elemen {element.id}
                      </div>
                    </div>
                  </div>
        ...
        {selectedElement && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              <div className={`bg-gradient-to-r ${getCategoryColor(selectedElement.category)} p-6 text-white`}>
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center mb-2">
                      <span className="text-3xl mr-3">{selectedElement.icon}</span>
                      <span className="text-lg font-semibold">
                        Elemen {selectedElement.id} - {getCategoryLabel(selectedElement.category)}
                      </span>
                    </div>
              ...
              <div className="p-6">
                <p className="text-gray-700 mb-4">
                  Detail implementasi untuk elemen {selectedElement.id} dalam sistem izin kerja selamat.
                  Elemen ini merupakan bagian dari fase {getCategoryLabel(selectedElement.category)} 
                  dalam siklus PDCA.
                </p>
                
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="font-semibold text-gray-800 mb-2">Kategori:</h4>
                  <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium text-white ${selectedElement.color}`}>
                    {getCategoryLabel(selectedElement.category)} - {selectedElement.title}
                  </span>
                </div>
              </div>
            </div>
